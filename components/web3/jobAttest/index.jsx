...
const schemaUID = process.env.NEXT_PUBLIC_JOB_ATTEST_SCHEMA_UID

export const readJobAttest = async (jobAttestUid) => {
    if (jobAttestUid == null) {
        return false;
    }

    if (typeof(jobAttestUid) != "string") {
        return false;
    }

    if (jobAttestUid.length != 66) {
        return false;
    }
    ...
    const attestation = await eas.getAttestation(jobAttestUid);
    console.log(attestation);

    return attestation;
}

export const createJobAttest = async (postId, postOwnerAddress) => {
    ...
    const schemaEncoder = new SchemaEncoder("string postId,bool jobDone");
    const encodedData = schemaEncoder.encodeData([
    { name: "postId", value: postId, type: "string" },
    { name: "jobDone", value: true, type: "bool" },
    ]);
